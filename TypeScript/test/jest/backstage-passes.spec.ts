import { BACKSTAGE_PASSES, afterOneDay } from './helpers';

describe('Backstage passes', () => {
  it('increases in quality as the concert approaches', () => {
    const item = afterOneDay(BACKSTAGE_PASSES, 20, 20);
  ...
  describe('threshold transitions', () => {
    it.each([
      ...
      const item = afterOneDay(BACKSTAGE_PASSES, sellIn, 20);
  ...
  describe('after the concert', () => {
    it('drops quality to 0 on the day the concert passes', () => {
      const item = afterOneDay(BACKSTAGE_PASSES, 0, 20);
    ...
    it('keeps quality at 0 for an already expired pass', () => {
      const item = afterOneDay(BACKSTAGE_PASSES, -1, 20);
    ...
    it('makes even a pass worth the maximum worthless', () => {
      expect(afterOneDay(BACKSTAGE_PASSES, 0, 50).quality).toBe(0);
    });
  });
  ...
  describe('quality never exceeds 50', () => {
    ...
    it.each([
      ...
      expect(afterOneDay(BACKSTAGE_PASSES, sellIn, quality).quality).toBe(50);
    });
  });
});
